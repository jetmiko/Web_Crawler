    ...
    async with AsyncWebCrawler() as crawler:
        ...
        try:
            ...
            if file_result and file_result.success:
                ...
                print("\nLocal file processing failed: Crawl did not return a valid result.")
                raise Exception("Crawl failed")  # Trigger fallback
        except Exception as e:
            print(f"\nLocal file processing error: {e}")
            # Fallback: Read file directly and process as raw HTML
            print("Falling back to reading local file directly...")
            try:
                with open(file_path, "r") as f:
                    local_html = f.read()
                fallback_result = await crawler.arun(
                    url="raw:" + local_html, config=CrawlerRunConfig(cache_mode=CacheMode.BYPASS)
                )
                if fallback_result and fallback_result.success:
                    print("Fallback local file processing:")
                    print(f"  Markdown: {fallback_result.markdown.raw_markdown[:50]}...")
                else:
                    print("Fallback processing failed: Crawl did not return a valid result.")
            except Exception as fallback_e:
                print(f"Fallback processing failed: {fallback_e}")

    # Clean up
    try:
        if file_path.exists():
            os.remove(file_path)
            print(f"Processed and cleaned up file ({file_path})")
    except OSError as e:
